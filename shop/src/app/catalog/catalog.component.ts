...
export class CatalogComponent implements OnInit {
  ...
  addToCart(productToCart: Product) {
    const productIndexInCart = (this.inCart.length > 0) ? this.inCart.findIndex(x => x.product.id === productToCart.id) : -1;
  ...
  deleteProductFromCart(productId: number) {
    const productIndexInCart = this.inCart.findIndex(x => x.product.id === productId);
